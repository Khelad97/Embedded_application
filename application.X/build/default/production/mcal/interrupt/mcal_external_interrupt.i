...
void interrupt_external_enable(const ext_int_config *int_config){
    if(int_config->int_source == EXTERNAL_INT0){
        ...
        }else{ INTCON2bits.INTEDG1 = INTERRUPT_FALLING_EDGE; }
        if(int_config->priority == HIGH_PRIORITY){
            INTCON3bits.INT1IP = HIGH_PRIORITY;
        }else{ INTCON3bits.INT1IP = LOW_PRIORITY; }
        INT1_InterruptHandler = int_config->EXT_InterruptHandler;
        gpio_pin_direction_intialize(int_config->port_name, int_config->pin, DIRECTION_INPUT);
        ...
        }else{ INTCON2bits.INTEDG2 = INTERRUPT_FALLING_EDGE; }
        if(int_config->priority == HIGH_PRIORITY){
            INTCON3bits.INT2IP = HIGH_PRIORITY;
        } else{ INTCON3bits.INT2IP = LOW_PRIORITY; }
        INT2_InterruptHandler = int_config->EXT_InterruptHandler;
        gpio_pin_direction_intialize(int_config->port_name, int_config->pin, DIRECTION_INPUT);
