...
# 12 "mcal/timers/mcal_timer0.h" 2
...
# 13 "mcal/timers/mcal_timer0.h" 2
...
ret_status timer0_initilize(const timer0_t *timer) {
    ...
    if (timer->timer_mode == 1) {
        (T0CONbits.T0CS=0);
    } else {
        (T0CONbits.T0CS=1);
        if (timer->timer_current_edge_mode == 1) {
            (T0CONbits.T0SE=1);
        } else {
            (T0CONbits.T0SE=0);
        }
    ...
    if (timer->timer_prescaler_mode == 1) {
        (T0CONbits.PSA=0);
...
ret_status timer0_read_timer(uint16_t *timer_value) {
    ret_status ret = R_OK;
    uint8_t read_tmr0l = ((uint8_t )0U);
    uint8_t read_tmr0h = ((uint8_t )0U);
    read_tmr0l = TMR0L;
    read_tmr0h = TMR0H;
